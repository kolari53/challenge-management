...
public class ChallengeService {
    ...
    @Transactional
    public Challenge createChallenge(String msisdn, String account, String lang) throws InterruptedException {
        ...
        if (repo.existsByMsisdnAndStatusIn(msisdn,
                List.of(ChallengeStatus.PENDING, ChallengeStatus.ACKNOWLEDGED))) {
        ...
        c.setStatus(ChallengeStatus.ACKNOWLEDGED);
        repo.save(c);
        Thread.sleep(5000);
        audit.record(c, ChallengeStatus.ACKNOWLEDGED, "PORTABILITY", "Challenge verified and acknowledged");

        notification.sendChallengeCreated(c);

        c.setStatus(ChallengeStatus.PENDING);
        repo.save(c);
        audit.record(c, ChallengeStatus.PENDING, "SYSTEM", "Notification sent, awaiting customer reply");
    ...
    public Challenge reply(String msisdn, String reply) {
        ...
        boolean accepted = switch (reply.trim().toUpperCase()) {
            case "OUI", "O", "YES", "Y","EVET" -> true;
            case "NON", "NO", "N","HAYIR" -> false;
            default -> {
